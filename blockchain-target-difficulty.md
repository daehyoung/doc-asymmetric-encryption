...
2. **실제 걸린 시간 예시**:
   - 예를 들어, 2,016개의 블록을 생성하는 데 실제로 18,144분이 걸렸다면:
     - 새로운 난이도 = 이전 난이도 × ( 20,160/18,144) = 이전 난이도 ×  
     - 난이도가 10% 감소합니다.

3. **반대로, 시간이 더 걸린 경우**:
   - 예를 들어, 2,016개의 블록을 생성하는 데 실제로 22,176분이 걸렸다면:
     - 새로운 난이도 = 이전 난이도 × ( 20,160/22,176) = 이전 난이도 ×  
     - 난이도가 10% 증가합니다.
...
맞습니다. 실제 경과 시간이 목표 시간인 20,160분보다 짧다면, 블록이 예상보다 더 빨리 생성되었음을 의미하고, 이 경우 난이도는 증가해야 합니다. 이전 설명에서 난이도 조정 예시가 잘못 설명되었습니다. 정확하게는 실제 경과 시간이 목표 시간보다 짧으면 난이도가 증가하고, 더 오래 걸렸다면 난이도가 감소해야 합니다. 올바른 예시는 다음과 같습니다.
